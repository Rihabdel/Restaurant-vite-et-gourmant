...
async function displayDishes(dishes) {
  ...
  dishes.forEach((dish) => {
    ...
    row.innerHTML = `
            ...
            <td>
                <button class="btn btn-primary btn-sm edit-dish-btn" title="Modifier" data-id="${sanitizeHtml(dish.id)}">
                    <i class="bi bi-pencil-square"></i>
                </button>
                <button class="btn btn-danger btn-sm delete-dish-btn" title="Supprimer" data-id="${sanitizeHtml(dish.id)}">
...
function initDishListeners() {
  ...
  const modalDelete = new bootstrap.Modal(
    document.getElementById("deleteDishModal"),
  );
  dishList.addEventListener("click", async (e) => {
    ...
    if (addAllergenBtn) {
      ...
      const select = document.querySelector(
        `select.allergen-select[data-id="${dishId}"]`,
      );
      if (!select) {
        console.error(
          "Sélecteur d'allergènes introuvable pour le plat ID :",
          dishId,
        );
  ...
  // afficher la modale d'ajout d'un allergène
  ...
  if (addAllergenButton) {
    addAllergenButton.addEventListener("click", () => {
      const modal = new bootstrap.Modal(
        document.getElementById("addAllergenModal"),
      );
      modal.show();
    });
  }
  // ajouter un allergen dans la base de donnée
  const addAllergenForm = document.getElementById("addAllergenForm");
  if (addAllergenForm) {
    addAllergenForm.addEventListener("submit", async (e) => {
      e.preventDefault();
      const formData = new FormData(addAllergenForm);
      const allergenData = {
        name: sanitizeHtml(formData.get("allergenName")),
        description: sanitizeHtml(formData.get("allergenDescription")),
      };
      try {
        await addAllergen(allergenData);
        alert("Allergène ajouté !");
        const modalEl = document.getElementById("addAllergenModal");
...
function initMenuEvents() {
  document.addEventListener("click", async (e) => {
    ...
    const current =
      btn.dataset.isAvailable === "1" || btn.dataset.isAvailable === "true";
    if (btn) {
      try {
  ...
  menuList.addEventListener("click", async (e) => {
    ...
    if (editBtn) {
      ...
      if (id) {
        ...
        console.error(
          "ID de menu non trouvé pour le bouton de modification cliqué.",
        );
      }
    }
    ...
    if (deleteDishMenuBtn) {
      ...
      const select = document.querySelector(
        `select.dish-select[data-id="${menuId}"]`,
      );
      if (!select) {
        console.error(
...
function initEmployeeListeners() {
  ...
  const inputConfirmPassword = formEmployee.querySelector(
...
async function newEmployee(event) {
  ...
  try {
    ...
    if (!response.ok)
      throw new Error(
        `Erreur ${response.status} lors de la création de l'employé`,
      );
...
async function loadEmployees() {
  try {
    ...
    if (!response.ok)
      throw new Error(
        `Erreur ${response.status} lors du chargement des employés`,
      );
...
function deleteEmployee(employeeId) {
  return fetch(`${API_BASE}/admin/employees/${employeeId}`, {
    ...
    if (!response.ok) {
      throw new Error(
        `Erreur ${response.status} lors de la suppression de l'employé`,
      );
    }
  });
}
...
export function fillEditOrderModal(order) {
  ...
  menu
      ...
      if (menuSelect) {
        menuSelect.innerHTML =
          ...
          menus
            .map(
              (menu) =>
                `<option value="${sanitizeHtml(menu.id)}" ${sanitizeHtml(order.menu?.id) === sanitizeHtml(menu.id) ? "selected" : ""}>${sanitizeHtml(menu.title)}</option>`,
            )
            .join("");
      }
    })
    .catch((error) => {
      console.error(
        "Erreur lors du chargement des menus pour la modale :",
        error,
      );
...
if (editOrderForm) {
  editOrderForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      const orderData = {
        numberOfPeople:
          sanitizeHtml(formData.get("numberOfPeople")) ||
          sanitizeHtml(existingOrder.numberOfPeople),
        ...
        canceledBy: getToken()
          ? "admin"
          : sanitizeHtml(existingOrder.canceledBy),
      };
  ...
  try {
    ...
    filteredOrders
      ...
      .catch((error) => {
        ...
        alert(
          "Aucune commande trouvée avec les critères de filtre sélectionnés.",
        );
      });
  } catch (error) {
    ...
    .then((menus) => {
      menu.innerHTML =
        ...
        menus
          .map(
            (menu) =>
              `<option value="${sanitizeHtml(menu.id)}">${sanitizeHtml(menu.title)}</option>`,
          )
          .join("");
    })
    .catch((error) => {
      console.error(
        "Erreur lors du chargement des menus pour le filtre :",
        error,
      );
      alert(
        "Une erreur est survenue lors du chargement des menus pour le filtre.",
      );
    });
}
...
function loadStatistics() {
  ...
  if (totalRevenue) {
    getStatictics()
      .then((statistics) => {
        totalRevenue.textContent = `${sanitizeHtml(statistics.totalRevenue)} €`;
      })
      .catch((error) => {
        console.error("Erreur lors du chargement des statistiques :", error);
        totalRevenue.textContent = "Erreur de chargement";
      });
  }
}
...
const menuChartConfig = {
  type: "bar",
  data: {
    labels: await getMenus()
      .then((menus) => menus.map((menu) => sanitizeHtml(menu.title)))
      .catch((error) => {
        console.error(
          "Erreur lors du chargement des menus pour le graphique :",
          error,
        );
        return [];
      }),
    datasets: [
      {
        ...
        data: await getOrdersAdmin()
          ...
          .catch((error) => {
            console.error(
              "Erreur lors du chargement des commandes pour le graphique :",
              error,
            );
            return [];
          }),
...
const chiffreAffairesByMenu = document
  .getElementById("chiffreAffairesByMenu")
  .getContext("2d");
const chiffreAffairesConfig = {
  type: "bar",
  data: {
    labels: await getMenus()
      .then((menus) => menus.map((menu) => menu.title))
      .catch((error) => {
        console.error(
          "Erreur lors du chargement des menus pour le graphique :",
          error,
    ...
    datasets: [
      {
        ...
        data: await getOrdersAdmin()
          .then((orders) => {
            ...
            orders.forEach((order) => {
              const menuTitle = order.menu?.title || "Inconnu";
              menuCounts[menuTitle] =
                (menuCounts[menuTitle] || 0) +
                (order.menu?.price || 0) * order.numberOfPeople;
            });
            return Object.values(menuCounts);
          })
          .catch((error) => {
...
const chiffreAffairesChart = new Chart(
  chiffreAffairesByMenu,
  chiffreAffairesConfig,
);
...
function initContactMsgListeners() {
  ...
  contactMessagesTable.addEventListener("click", async (e) => {
    ...
    if (deleteBtn) {
      ...
      const modal = new bootstrap.Modal(
        document.getElementById("deleteContactMsgModal"),
      );
      const confirmDeleteBtn = document.getElementById(
        "confirmDeleteContactMsgBtn",
      );
...
function displayContactMsg(messages) {
  ...
  contactMessagesTable.innerHTML = messages
    .map(
      (msg) => `
        <tr>
