...
export const API_BASE = "https://vite-gourmand-api-novo.onrender.com/api";
...
export async function getDishAllergens(dishId) {
  ...
  if (!response.ok) {
    const errorData = await response.json();
    throw new Error(
      errorData.message || "Erreur de mise à jour de la commande",
    );
  }
  return await response.json();
}
...
export async function addDishAllergens(dishId, allergenIds) {
  ...
  if (!response.ok) {
    const errorData = await response.json();
    throw new Error(
      errorData.message || "Erreur d'ajout des allergènes au plat",
    );
  }
  return await response.json();
}
...
export async function getOrdersByUserId() {
  ...
  if (!response.ok)
    throw new Error("Erreur chargement commandes de l'utilisateur");
  return await response.json();
}
...
export async function cancelOrder(id) {
  ...
  if (!response.ok) {
    const errorData = await response.json();
    throw new Error(
      errorData.message || "Erreur de suppression de la commande",
    );
  }
  return await response.json();
}
...
export async function deleteDishFromMenu(menuId, dishId) {
  const response = await fetch(
    `${API_BASE}/menus-dishes/${menuId}/remove-dish/${dishId}`,
    {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    const errorData = await response.json();
    throw new Error(
      errorData.message || "Erreur de suppression du plat du menu",
    );
  }
  return await response.json();
}
