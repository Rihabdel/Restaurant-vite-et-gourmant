...
function inituserListener() {
  ...
  const inputConfirmPassword = formInscription.querySelector(
...
export function newUser(event) {
  ...
  const raw = JSON.stringify({
    ...
    address:
      sanitizeHtml(document.getElementById("AddressePostaleInput")?.value) ||
      "",
  ...
  fetch(`${API_BASE}/register`, requestOptions)
    ...
    .then((result) => {
      const firstName =
        sanitizeHtml(document.getElementById("PrenomInput")?.value) || "";
      alert(
        `Bravo ${firstName}, vous êtes maintenant inscrit, vous pouvez vous connecter.`,
      );
