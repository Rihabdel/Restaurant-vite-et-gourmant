...
function initButtons() {
  ...
  if (editProfileBtn) {
    editProfileBtn.addEventListener("click", () => {
      const isExpanded =
        editProfileBtn.getAttribute("aria-expanded") === "true";
  ...
  if (profileForm) {
    profileForm.addEventListener("submit", async (event) => {
      ...
      const updatedData = {
        ...
        phone:
          parseInt(formData.get("NumeroTelephone").replace(/\D/g, "")) || null,
...
async function deleteAccount() {
  ...
  alert(
    "Votre compte a été supprimé. Vous allez être redirigé vers la page d'accueil.",
  );
  signout();
}
async function LoadProfil() {
  try {
    const user = await getUserInfo();
    console.log(
      "Données de l'utilisateur récupérées pour modification :",
      user,
    );
