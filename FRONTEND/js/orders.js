...
async function loadOrders() {
  ...
  try {
    ...
    console.log(
      "Informations utilisateur récupérées pour chargement des commandes :",
      user,
    );
                ...
                <td>${new Date(order.deliveryDate).toISOString().split("T")[0]}</td>
                    ...
                    <td>${
                      order.status === "en attente"
                        ? `<button class="btn btn-sm btn-primary edit-order-btn" data-id="${sanitizeHtml(order.id)}"><i class="bi bi-pencil"></i></button>`
                        : "Non modifiable"
                    }</td>
                    <td>${order.status === "en attente" ? `<button class="btn btn-sm btn-danger cancel-order-btn" data-id="${sanitizeHtml(order.id)}"><i class="bi bi-x"></i></button>` : "Non annulable"}</td>
                    <td>
...
function fillEditOrderModalUser(orderId) {
  ...
  console.log(
    "Récupération des détails de la commande pour modification, ID :",
    orderId,
  );
  getOrderById(orderId)
    .then((order) => {
      ...
      livraisonDate.value = order.deliveryDate
    ...
    .catch((error) => {
      console.error(
        "Erreur lors du chargement des informations de la commande :",
        error,
      );
      alert(
...
function initButtons() {
  ...
  if (newOrderBtn) {
    newOrderBtn.addEventListener("click", async () => {
      try {
        ...
        console.error(
          "Erreur lors de l'ouverture de la modale de création de commande :",
          error,
        );
        alert(
          "Une erreur est survenue lors de l'ouverture du formulaire de commande. Veuillez réessayer.",
        );
      }
    });
  }
  ...
  if (confirmOrderBtn) {
    confirmOrderBtn.addEventListener("click", async () => {
      ...
      console.log(
        "Données de la commande récupérées du formulaire :",
        orderData,
      );
  ...
  if (payOrderBtn) {
    payOrderBtn.addEventListener("click", () => {
      ...
      createOrder(data)
        .then(() => {
          alert(
            "un email de confirmation de commande vous a été envoyé, merci pour votre commande!",
          );
  ...
  if (editOrderForm) {
    editOrderForm.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        alert(
          "Une erreur est survenue lors de la mise à jour de votre commande. Veuillez réessayer.",
        );
      }
    });
  }
...
export async function fillNewOrderModal() {
  try {
    ...
    console.log(
      "Informations utilisateur pré-remplies dans la modale de création de commande :",
      user,
    );
    const modal = bootstrap.Modal.getOrCreateInstance(
    ...
    console.error(
      "Erreur lors du remplissage de la modale de création de commande :",
      error,
    );
    alert(
        ...
        <h3 class="text-center mb-4">Récapitulatif de votre commande</h3>
            ...
            <h4 class="mb-3">Informations de livraison</h4>
                ...
                <p><strong>Heure de livraison :</strong> ${
                  sanitizeHtml(orderData.deliveryTime)
                    ? new Date(orderData.deliveryTime)
                        .toTimeString()
                        .slice(0, 5)
                    : "N/A"
                }</p>
...
export async function fillEditOrderModal(orderId) {
  try {
    const user = await getUserInfo();
    console.log(
      "Informations utilisateur récupérées pour modification de commande :",
      user,
    );
    ...
    document.getElementById("customerName").value =
      sanitizeHtml(order.user.firstName) || "";
